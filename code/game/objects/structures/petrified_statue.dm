...
#define PETRIFIED_TRAIT_SOURCE "petrified_statue"
...
/obj/structure/statue/petrified/Destroy()
	...
	if(petrified_mob)
		...
		petrified_mob.remove_traits(list(TRAIT_MUTE), STATUE_MUTE)
		REMOVE_TRAIT(petrified_mob, TRAIT_BLOODLOSS_IMMUNE, PETRIFIED_TRAIT_SOURCE)
...
/obj/structure/statue/petrified/deconstruct(disassembled = TRUE)
	...
	if(!disassembled)
		if(petrified_mob)
			log_game("[key_name(petrified_mob)] was dusted by petrified statue deconstruction at [AREACOORD(src)].")
...
/mob/living/carbon/human/petrify(statue_timer)
	...
	ADD_TRAIT(src, TRAIT_BLOODLOSS_IMMUNE, PETRIFIED_TRAIT_SOURCE)
	S.icon_state = null
	S.copy_overlays(src)
