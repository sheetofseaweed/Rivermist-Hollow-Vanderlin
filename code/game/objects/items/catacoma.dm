...
/obj/item/book/secret/ledger/Initialize(mapload)
	. = ..()
	SSmerchant.register_ledger(src)

/obj/item/book/secret/ledger/Destroy()
	SSmerchant.unregister_ledger(src)
	cart.Cut()
	return ..()
...
/obj/item/book/secret/ledger/proc/on_faction_changed()
	cart.Cut()

/obj/item/book/secret/ledger/proc/can_order_pack(datum/supply_pack/pack, datum/world_faction/faction = SSmerchant.active_faction)
	if(!pack || !faction || faction.faction_supply_packs[pack.type] != pack)
		return FALSE
	if(pack.hidden || (pack.special && !pack.special_enabled))
		return FALSE
	if(pack.contraband && !istype(src, /obj/item/book/secret/ledger/fence))
		return FALSE
	return TRUE

/obj/item/book/secret/ledger/proc/get_pack_unit_price(datum/supply_pack/pack)
	if(istype(src, /obj/item/book/secret/ledger/fence) && !pack.contraband)
		return FLOOR(pack.cost * 1.5, 1)
	return pack.cost

/obj/item/book/secret/ledger/proc/resolve_orderable_pack(pack_ref)
	var/datum/world_faction/faction = SSmerchant.active_faction
	var/datum/supply_pack/pack = locate(pack_ref)
	if(!can_order_pack(pack, faction))
		return null
	return pack
...
/obj/item/book/secret/ledger/ui_data(mob/user)
	...
	if(faction && islist(faction.active_bounties))
		for(var/datum/bounty/bounty in faction.active_bounties)
			...
			if(bounty.required_reagent_type)
	...
	for(var/datum/supply_pack/pack in cart)
		...
		var/item_mammon = get_pack_unit_price(pack) * quantity
...
/obj/item/book/secret/ledger/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("add_to_cart")
			var/datum/supply_pack/pack = resolve_orderable_pack(params["id"])
			if(!pack)
				return TRUE
		...
		if("submit_order")
			create_order_scroll(ui.user)
			return TRUE

		if("reroll_bounty")
			var/datum/bounty/target = locate(params["id"])
			var/datum/world_faction/faction = SSmerchant.active_faction
			if(!target || !faction || !(target in faction.active_bounties))
				return TRUE
			faction.reroll_bounty(target, ui.user)
			return TRUE

		if("manual_rotate_faction")
			var/datum/world_faction/chosen = locate(params["ref"])
			if(!chosen || !(chosen in SSmerchant.world_factions))
				return TRUE
			SSmerchant.manual_rotate_faction(chosen, ui.user)
			return TRUE
...
/obj/item/book/secret/ledger/proc/create_order_scroll(mob/current_reader)
	if(!length(cart))
		to_chat(current_reader, "<span class='warning'>Your cart is empty!</span>")
		return

	var/datum/world_faction/faction = SSmerchant.active_faction
	if(!faction)
		to_chat(current_reader, "<span class='warning'>No active faction found!</span>")
		return

	var/obj/item/paper/scroll/cargo/order = new(get_turf(current_reader))
	order.orders = cart.Copy()

	// Track the active faction on the scroll instance
	order.buying_from = faction
	order.fence_order = istype(src, /obj/item/book/secret/ledger/fence)
	...
	for(var/datum/supply_pack/pack in cart)
		var/quantity = cart[pack]
		var/quoted_price = get_pack_unit_price(pack)
		order.quoted_prices[pack] = quoted_price
		total_mammon_cost += quoted_price * quantity

	to_chat(current_reader, "<span class='notice'>Order scroll created! Cost: [total_mammon_cost] mammons from [faction.faction_name]</span>")
	order.rebuild_info()
