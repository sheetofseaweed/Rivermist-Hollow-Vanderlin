...
/datum/atom_hud/alternate_appearance/hide_from(mob/former_viewer, absolute)
	. = ..()
	if(!former_viewer || hud_users_all_z_levels[former_viewer] >= 1)
		return
...
/datum/atom_hud/alternate_appearance/basic/people/New(key, image/I, options = NONE, list/mob/seers)
	var/list/candidate_seers = islist(seers) ? seers : list(seers)
	src.seers = list()
	for(var/mob/candidate as anything in candidate_seers)
		src.seers += WEAKREF(candidate)
	return ..()
