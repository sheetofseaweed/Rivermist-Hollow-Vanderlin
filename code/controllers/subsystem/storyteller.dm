...
/datum/controller/subsystem/gamemode/proc/refresh_alive_stats(roundstart = FALSE, first_post_roundstart_check = FALSE)
	...
	for(var/client/client in GLOB.clients)
		...
		if(ishuman(living))
			...
			if(human_mob.mind.assigned_role.title in GLOB.townwatch_positions)
				record_round_statistic(STATS_ALIVE_GARRISON)
			if(human_mob.mind.assigned_role.title in GLOB.chapel_positions)
				record_round_statistic(STATS_ALIVE_CLERGY)
			if((human_mob.mind.assigned_role.title in GLOB.traders_positions))
	...
	if(valid_psydon_favourite)
		set_chronicle_stat(CHRONICLE_STATS_PSYDON_FAVOURITE, valid_psydon_favourite, "AO'S CHOSEN", "#e6e6e6", "buying their way in")
