...
/turf/proc/link_below(turf/new_below, skip_reciprocal = FALSE)
	if(virtual_below)
		...
		virtual_below = null
		var/list/link_directions = GLOB.virtual_z_links[src]
		link_directions?.Remove("below")
...
/proc/unlink_region(turf/bottom_left, width, height)
	...
	for(var/x in 0 to width - 1)
		for(var/y in 0 to height - 1)
			var/turf/linked_turf = locate(bottom_left.x + x, bottom_left.y + y, bottom_left.z)
			linked_turf?.unlink_virtual_z()
