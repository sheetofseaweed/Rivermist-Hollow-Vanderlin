...
/datum/mind/proc/recall_targets(mob/recipient, window=1)
	...
	for (var/mob/living/carbon in GLOB.mob_living_list) // Iterate through all mobs in the world
		if (HAS_TRAIT(carbon, TRAIT_ZIZOID_HUNTED) && (!istype(carbon, /mob/living/carbon/human/dummy)))//To be on the list they must be hunted, not be the user and not be a dummy (There is a dummy that has all vices for some reason)
