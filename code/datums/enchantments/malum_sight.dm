...
/datum/enchantment/malum_sight/proc/on_equip(obj/item/i, mob/living/user, slot)
	...
	if(active_item)
		...
		to_chat(user, span_notice("The stone around me comes into focus."))
...
/datum/enchantment/malum_sight/unregister_triggers()
...
/mob/living/proc/set_oresight(state)
	...
	if(!COS)
		COS = AddComponent(/datum/component/ore_sight)
	COS?.toggle(state)
...
/datum/component/ore_sight/Destroy(force)
	STOP_PROCESSING(SSdcs, src)
	var/mob/living/user = parent
	user?.remove_status_effect(/datum/status_effect/buff/oresight)
	return ..()
