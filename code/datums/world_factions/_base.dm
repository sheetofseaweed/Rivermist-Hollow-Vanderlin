/datum/world_faction
	...
	/// Earliest time an emptied bounty slot may be refilled.
	var/bounty_refill_ready_time = 0
...
/datum/world_faction/Destroy()
	QDEL_LIST_ASSOC_VAL(faction_supply_packs)
	QDEL_LIST(active_bounties)
	QDEL_LIST(next_boat_traders)
...
/datum/world_faction/proc/fluctuate_bounties()
	...
	if(length(active_bounties) >= max_active_bounties)
		return
	if(world.time < bounty_refill_ready_time)
		return
...
/datum/world_faction/proc/award_supply_reputation(datum/supply_pack/pack)
	...
	return get_reputation_tier() > old_tier
...
/datum/world_faction/proc/get_reagent_sell_values(atom/movable/container)
	var/list/values = list()
	if((!istype(container, /obj/item/reagent_containers) && !istype(container, /obj/structure)) || !container.reagents)
		return values
...
/datum/world_faction/proc/get_actual_sell_price(atom/sell_type, sell_modifier = 1)
	...
	var/static_modifier = get_hard_value_multiplier(sell_type)
...
/datum/world_faction/proc/return_sell_modifier(atom/sell_type)
	...
	return base_modifier * static_modifier

/// Returns the configured multiplier for this exact type or a configured parent family.
/datum/world_faction/proc/get_hard_value_multiplier(atom/sell_type)
	if(sell_type in hard_value_multipliers)
		return hard_value_multipliers[sell_type]
	for(var/parent_type in hard_value_multipliers)
		if(ispath(sell_type, parent_type))
			return hard_value_multipliers[parent_type]
	return 1
...
/datum/world_faction/proc/handle_selling_single(atom/movable/selling_item)
	for(var/datum/bounty/bounty in active_bounties)
		...
		if(bounty_result == BOUNTY_MATCH_CONSUMED)
			...
			var/bounty_complete = bounty.required_reagent_type ? (bounty.current_reagent_amount >= bounty.required_reagent_amount) : (bounty.current_count >= bounty.required_count)
			if(bounty_complete)
				bounty.fulfill_bounty(src)
				complete_bounty(bounty)
			return TRUE
...
/datum/world_faction/proc/complete_bounty(datum/bounty/completed_bounty)
	active_bounties -= completed_bounty
	bounty_refill_ready_time = max(bounty_refill_ready_time, world.time + bounty_refill_cooldown)
	qdel(completed_bounty)
...
/datum/world_faction/proc/get_faction_quality_calculator(obj/item/target)
	...
	if(istype(target, /obj/item/reagent_containers/glass/bottle))
		calc = new /datum/quality_calculator/brewing(
			mat_qual = SMELTERY_LEVEL_NORMAL + round(tier / 2),
			skill_qual = tier,
			components = 1,
			reagent_qual = 0,
			fresh = 5 MINUTES,
		...
		var/quality_peak = round(-1 + (5 * tier / 6))
		rolled_quality = roll_bell_quality(quality_peak, spread = 2, low_bound = -1, high_bound = 4)
		...
		calc = new /datum/quality_calculator/cooking(
			mat_qual = SMELTERY_LEVEL_POOR + round(tier / 2),
			skill_qual = tier,
			components = 1,
		...
		var/datum/quality_calculator/blacksmithing/smithing_quality = new(
			mat_qual = SMELTERY_LEVEL_NORMAL + round(tier / 2),
		...
		// RMH's calculator stores successful minigame performance separately;
		// feeding it into perf_qual would be interpreted as a crafting penalty.
		smithing_quality.minigame_success = 60 + (tier * 5)
		calc = smithing_quality
		var/quality_peak = round(text2num(QUALITY_LEVEL_SPOILED) + (18 * tier / 6))
		rolled_quality = roll_bell_quality(quality_peak, spread = 2, low_bound = text2num(QUALITY_LEVEL_SPOILED), high_bound = text2num(QUALITY_LEVEL_LEGENDARY))
	if(!calc)
		return null
