/datum/bounty
	...
	// The type of item or path needed to fulfill the bounty
	var/required_path
...
/datum/bounty/proc/check_completion(atom/movable/delivered_item)
...
/datum/bounty/proc/check_reagent_completion(atom/movable/delivered_item)
	if(required_path && !istype(delivered_item, required_path))
		return BOUNTY_NO_MATCH

	if((!istype(delivered_item, /obj/item/reagent_containers) && !istype(delivered_item, /obj/structure)) || !delivered_item.reagents)
		return BOUNTY_NO_MATCH

	var/found_amount = delivered_item.reagents.get_reagent_amount(required_reagent_type)
	if(found_amount <= 0)
		return BOUNTY_NO_MATCH
	...
	delivered_item.reagents.remove_reagent(required_reagent_type, amount_to_take)
	...
	if(delivered_item.reagents.get_reagent_amount(required_reagent_type) <= 0)
