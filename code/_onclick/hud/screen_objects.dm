...
/atom/movable/screen/zone_sel/update_overlays()
	...
	if(hud.mymob.stat != DEAD && ishuman(hud.mymob))
		...
		for(var/obj/item/bodypart/BP as anything in H.bodyparts)
			...
			var/comparison = (damage/BP.max_damage)			// if taur bodypart, render the healthy/wounded/bleed overlays for both legs
			if(BP.body_zone == BODY_ZONE_TAUR)
				var/list/_legs = list(BODY_ZONE_L_LEG, BODY_ZONE_R_LEG)
				for(var/_z in _legs)
...
/atom/movable/screen/party_member_name/proc/set_party_member(mob/mob, datum/party/incoming_party, rank = "Recruit")
	...
	maptext = MAPTEXT("<span style='font-family: \"Small Fonts\"; font-size: 7px; color: #FFFFFF; text-shadow: 1px 1px 0px #000000;'>[display_name]<br><span style='color: #FFD700;'>[rank]</span></span>")
