...
/atom/movable/screen/plane_master/proc/relay_render_to_plane(mob/mymob, relay_plane)
	...
	relay = new()
	var/relay_layer = isnull(render_relay_layer) ? (plane + abs(LOWEST_EVER_PLANE)) * 0.5 : render_relay_layer
	relay.render_source = render_target
	relay.plane = relay_plane
	relay.layer = relay_layer //layer must be positive but can be a decimal
	...
	if(blend_mode != BLEND_MULTIPLY) //internal snowflake do not touch
