		...
		if(stealroll >= target_perception)
			...
			if(do_after(U, (2.5 - (thief_skill_base * 0.25)) SECONDS, V, progress = FALSE))
				switch(U.zone_selected)
					if(BODY_ZONE_HEAD)
						if (V.get_item_by_slot(ITEM_SLOT_EARRING_L))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_EARRING_L))
						if (V.get_item_by_slot(ITEM_SLOT_EARRING_R))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_EARRING_R))
					if(BODY_ZONE_CHEST)
						...
						if (V.get_item_by_slot(ITEM_SLOT_UNDER_TOP))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_UNDER_TOP))
						if (V.get_item_by_slot(ITEM_SLOT_UNDERSHIRT))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_UNDERSHIRT))
					if(BODY_ZONE_L_ARM)
						if (V.get_item_by_slot(ITEM_SLOT_BACK_L))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_BACK_L))
					if(BODY_ZONE_R_ARM)
					...
					if(BODY_ZONE_PRECISE_NECK)
						...
						if (V.get_item_by_slot(ITEM_SLOT_GARTER))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_GARTER))
					if(BODY_ZONE_PRECISE_GROIN)
						...
						if (V.get_item_by_slot(ITEM_SLOT_UNDER_BOTTOM))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_UNDER_BOTTOM))
						if (V.get_item_by_slot(ITEM_SLOT_GARTER))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_GARTER))
					if(BODY_ZONE_L_ARM)
						if (V.get_item_by_slot(ITEM_SLOT_BELT_L))
					...
					if(BODY_ZONE_R_LEG, BODY_ZONE_L_LEG)
						if (V.get_item_by_slot(ITEM_SLOT_SOCKS))
							stealpos.Add(V.get_item_by_slot(ITEM_SLOT_SOCKS))
