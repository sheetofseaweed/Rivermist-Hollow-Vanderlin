...
/datum/surgery_step/heal/validate_target(mob/user, mob/living/carbon/target, target_zone, datum/intent/intent)
	...
	if(iscarbon(target))
		...
		for(var/datum/injury/injury as anything in target.all_injuries)
			if(brute && burn)
				break
			if(injury.heals_as_brute_damage() && injury.required_status == BODYPART_ORGANIC)
				brute = TRUE
				continue
...
/datum/surgery_step/heal/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/intent/intent)
	...
	if(iscarbon(target))
		for(var/datum/injury/injury as anything in target.all_injuries)
			...
			if(urhealedamt_brute && injury.heals_as_brute_damage() && injury.required_status == BODYPART_ORGANIC)
	...
	target.defeat_try_prepared_recovery(user, "surgery", tool)
