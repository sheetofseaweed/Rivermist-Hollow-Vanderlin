...
/obj/item/organ/heart/cursed/update_icon_state()
	. = ..()
	icon_state = "cursedheart-[beating ? "on" : "off"]"

/obj/item/organ/heart/cursed/attack(mob/living/carbon/target, mob/user, list/modifiers)
...
/obj/item/organ/heart/cursed/on_life(delta_time, times_fired)
	...
	if(ishuman(owner) && owner.client)
		...
		if(human_owner.dna && !(NOBLOOD in human_owner.dna.species.species_traits))
			human_owner.blood_volume = max(human_owner.blood_volume - blood_loss, 0)
			to_chat(human_owner, span_danger("I have to keep pumping my blood!"))
			if(add_colour)
				human_owner.add_client_colour(/datum/client_colour/cursed_heart_blood)
				add_colour = FALSE
	else
...
/datum/action/item_action/organ_action/cursed_heart/Trigger(trigger_flags)
	...
	if(. && istype(target, /obj/item/organ/heart/cursed))
		...
		var/mob/living/carbon/human/human_owner = owner
		if(istype(human_owner) && human_owner.dna && !(NOBLOOD in human_owner.dna.species.species_traits))
			human_owner.blood_volume = min(human_owner.blood_volume + cursed_heart.blood_loss * 0.5, BLOOD_VOLUME_MAXIMUM)
			human_owner.remove_client_colour(/datum/client_colour/cursed_heart_blood)
			cursed_heart.add_colour = TRUE
			human_owner.adjustBruteLoss(-cursed_heart.heal_brute)
			human_owner.adjustFireLoss(-cursed_heart.heal_burn)
			human_owner.adjustOxyLoss(-cursed_heart.heal_oxy)
