...
/obj/item/bodypart/proc/update_injuries(delta_time, times_fired)
	...
	if(post_damage_change(FALSE) && owner)
...
/obj/item/bodypart/proc/update_germs(delta_time, times_fired, virus_immunity, antibiotics, immunity_weakness)
	//Cryo stops germs from moving and doing their bad stuffs
	if(owner.bodytemperature <= -15)
		return
	handle_germ_sync(delta_time, times_fired, antibiotics)
...
/obj/item/bodypart/proc/handle_germ_sync(delta_time, times_fired, antibiotics)
...
/// Update damage totals and dependent limb state after injuries change.
...
/obj/item/bodypart/proc/get_specific_markings_overlays(list/specific_markings, aux = FALSE, mob/living/carbon/human/human_owner, override_color)
	...
	var/list/emissive_markings = human_owner.dna.emissive_markings
...
/obj/item/bodypart/proc/get_cut(surgical_only = FALSE, ignore_gauze = FALSE)
	...
	var/static/list/cut_damage_types = list(WOUND_SLASH, WOUND_PIERCE, WOUND_BITE)
	var/datum/injury/internal_incision
	for(var/datum/injury/slash in injuries)
		if(!(slash.damage_type in cut_damage_types))
			continue
