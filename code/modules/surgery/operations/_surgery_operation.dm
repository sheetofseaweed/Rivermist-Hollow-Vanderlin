...
/datum/surgery_operation/proc/get_roll_requirement(atom/movable/operating_on, mob/living/surgeon, tool)
	var/skill_level = skill_used ? (GET_MOB_SKILL_VALUE(surgeon, skill_used) || 0) : SKILL_LEVEL_LEGENDARY
	var/requirement = get_base_roll_requirement_for_skill(skill_level) + (dice_requirement - 25)
	requirement += get_tool_quality_roll_modifier(get_tool_quality(tool))
	requirement += get_location_roll_modifier(get_location_modifier(get_turf(operating_on)))
	requirement += get_diceroll_roll_modifier(surgeon)
...
/// Translate RMH's medicine-skill success curve into a 3d20 roll requirement.
/datum/surgery_operation/proc/get_base_roll_requirement_for_skill(skill_level)
	skill_level = clamp(skill_level || 0, 0, SKILL_LEVEL_LEGENDARY)
	if(skill_level <= 15)
		return 22 + (skill_level * 0.6)
	if(skill_level <= 30)
		return 31 + ((skill_level - 15) * 0.6)
	if(skill_level <= 40)
		return 40 + ((skill_level - 30) * 0.8)
	return 48 + ((skill_level - 40) * 0.25)

/// Better (faster) tools improve success; slower or improvised tools penalize it.
/datum/surgery_operation/proc/get_tool_quality_roll_modifier(tool_quality)
	return clamp(round((1 - tool_quality) * 6, 0.1), -10, 5)

/// Preserve RMH's operating-table, bed, table, and floor success bonuses.
/datum/surgery_operation/proc/get_location_roll_modifier(location_modifier)
	if(location_modifier <= 1)
		return 3
	if(location_modifier <= 1.25)
		return 1
	if(location_modifier <= 1.5)
		return -2
	return -10

/// Preserve the physical diceroll modifiers used by RMH's former surgery steps.
/datum/surgery_operation/proc/get_diceroll_roll_modifier(mob/living/surgeon)
	if(!surgeon?.attributes)
		return 0
	return clamp(round(surgeon.attributes.get_diceroll_modifier(DICE_CONTEXT_PHYSICAL) * 2, 0.1), -15, 15)

/// Approximate RMH's low-skill critical-failure curve on the 3d20 distribution.
/datum/surgery_operation/proc/get_critical_failure_roll_threshold(skill_level)
	skill_level = clamp(FLOOR(skill_level || 0, 1), 0, SKILL_LEVEL_LEGENDARY)
	if(skill_level >= 15)
		return (dice_num * dice_sides) + 1

	var/static/list/thresholds = list(
		45, 45, 46, 46, 47,
		47, 48, 48, 49, 49,
		50, 51, 52, 53, 55,
	)
	return thresholds[skill_level + 1]

/// Roll a surgery result while keeping RMH's critical-failure cutoff independent of success modifiers.
/datum/surgery_operation/proc/roll_surgery_result(requirement, skill_level)
	var/dice = roll(dice_num, dice_sides)
	if(dice <= requirement)
		if(dice <= max(dice_num, requirement - dice_crit))
			return DICE_CRIT_SUCCESS
		return DICE_SUCCESS
	if(dice >= get_critical_failure_roll_threshold(skill_level))
...
/datum/surgery_operation/proc/get_overseer_bonus(mob/living/surgeon)
	...
	for(var/mob/living/carbon/human/nearby in oview(3, surgeon))
		...
		var/bonus = clamp(round((overseer_skill - max(our_skill, skill_median)) / 5, 0.1), 1, 5)
		if(bonus > best_bonus)
			best_bonus = bonus
...
/datum/surgery_operation/proc/try_perform(atom/movable/operating_on, mob/living/surgeon, tool, list/operation_args = list())
	...
	do
		...
		if(force_fail)
			...
			var/skill_level = skill_used ? (GET_MOB_SKILL_VALUE(surgeon, skill_used) || 0) : SKILL_LEVEL_LEGENDARY
			roll_result = roll_surgery_result(roll_requirement, skill_level)
