...
/datum/surgery_operation/basic/tend_wounds/state_check(mob/living/patient)
	...
	for(var/datum/injury/injury as anything in carbon_patient.all_injuries)
		...
		if(!injury.can_heal() || injury.is_surgical())
			continue
		if(injury.heals_as_brute_damage() || injury.damage_type == WOUND_BURN)
			return TRUE

	return FALSE

/datum/surgery_operation/basic/tend_wounds/snowflake_check_availability(mob/living/patient, mob/living/surgeon, tool, operated_zone)
	if(!isitem(tool))
		return TRUE

	var/obj/item/real_tool = tool
	if(real_tool.tool_behaviour != TOOL_SUTURE)
		return TRUE

	if(!iscarbon(patient))
		return patient.getBruteLoss() > 0
	return check_for_injuries(patient, TRUE, FALSE)
...
/datum/surgery_operation/basic/tend_wounds/proc/check_for_injuries(mob/living/carbon/patient, brute_check, burn_check)
	...
	for(var/datum/injury/injury as anything in carbon_patient.all_injuries)
		...
		if(!injury.can_heal() || injury.is_surgical())
			continue
		if(brute_check && injury.heals_as_brute_damage())
			brute_found = TRUE
		if(burn_check && injury.damage_type == WOUND_BURN)
			burn_found = TRUE
...
/datum/surgery_operation/basic/tend_wounds/get_radial_options(mob/living/patient, obj/item/tool, operating_zone)
	...
	var/using_suture = isitem(tool) && tool.tool_behaviour == TOOL_SUTURE
	...
	if(!using_suture && (can_heal & BURN_SURGERY) && check_for_injuries(patient, FALSE, TRUE))
...
/datum/surgery_operation/basic/tend_wounds/combo/tool_check(obj/item/tool)
	return tool.tool_behaviour != TOOL_SUTURE
