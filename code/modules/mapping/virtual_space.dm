...
/obj/effect/mapping_helpers/virtual_z_link/proc/do_link(obj/effect/mapping_helpers/virtual_z_link/above, obj/effect/mapping_helpers/virtual_z_link/below)
	var/width = above.link_width || below.link_width
	var/height = above.link_height || below.link_height
	...
	if(width && height)
		...
		for(var/turf/linked_turf as anything in above_area.get_turfs_by_zlevel(above.z))
			if(linked_turf.z != above.z)
				continue
			above_min_x = min(above_min_x, linked_turf.x)
			above_min_y = min(above_min_y, linked_turf.y)

		for(var/turf/linked_turf as anything in below_area.get_turfs_by_zlevel(below.z))
			if(linked_turf.z != below.z)
				continue
			below_min_x = min(below_min_x, linked_turf.x)
			below_min_y = min(below_min_y, linked_turf.y)
			below_max_x = max(below_max_x, linked_turf.x)
			below_max_y = max(below_max_y, linked_turf.y)
	...
	if(!width || !height || !above_origin || !below_origin)
		log_mapping("[src] at [AREACOORD(src)] could not determine a valid region for link_id '[link_id]'!")
		qdel(src)
		return
