...
/mob/living/proc/CheckEyewitness(atom/source, attacker_or_range = DEFAULT_MESSAGE_RANGE, range = DEFAULT_MESSAGE_RANGE, affects_source = FALSE)
	var/legacy_call = istype(attacker_or_range, /atom)
	if(!source)
		return legacy_call ? FALSE : list()
	var/atom/visibility_source = legacy_call ? attacker_or_range : source
	if(!legacy_call)
		range = attacker_or_range
	var/actual_range = max(1, round(range * (visibility_source.alpha / 255)))
	var/list/seenby = list()
	var/turf/T = get_turf(src)
	for(var/mob/living/carbon/human/human in oviewers(1, src))
		if(!human.affects_masquerade())
			continue
		if(T != get_step(human, turn(human.dir, 180)))
			seenby |= human
	for(var/mob/living/carbon/human/human in viewers(actual_range, source))
		...
		if(!human.affects_masquerade())
			continue
		if(!human.pulledby)
			var/source_is_visible = FALSE
			if(ismob(source))
				var/mob/source_mob = source
				source_is_visible = source_mob.has_light_nearby()
			else
				var/turf/source_turf = get_turf(source)
				if(source_turf && source_turf.get_lumcount() > LIGHTING_TILE_IS_DARK)
					source_is_visible = TRUE
			if(source_is_visible || human.has_nightvision() || get_dist(human, source) <= 1)
				if(human.can_see_cone(src))
					seenby |= human
	if(legacy_call)
		return length(seenby) > 0
	return seenby
...
/mob/living/carbon/human/proc/update_masquerade(value, forced = FALSE)
	if(!clan)
		return
	if(!forced)
		if(value > 0)
...
/mob/living/carbon/human/AdjustMasquerade(value, forced = FALSE)
	if(value < 0)
		vampire_detected(abs(value), forced)
	update_masquerade(value, forced)
