...
/mob/living/carbon/human/proc/update_damage_overlays_real()
	...
	if(use_female_sprites && !(gender == MALE && species.swap_male_clothes_but_not_offsets))
		...
		limb_icon = species.dam_icon_f
	else
		offsets = species.offset_features_m
		limb_icon = species.dam_icon_m
	...
	if(backr)
		if(backr.alternate_worn_layer == CLOAK_BEHIND_LAYER)
			...
			if(backr.experimental_onback && !hide_experimental)
				...
				back_overlay = backr.build_worn_icon(age, BACK_LAYER, 'modular_rmh/icons/clothing/onmob/back_r.dmi')
				if(LAZYACCESS(offsets, OFFSET_BACK))
					back_overlay.pixel_x += offsets[OFFSET_BACK][1]
					back_overlay.pixel_y += offsets[OFFSET_BACK][2]
				// Apply taur body offset to adjust rendering height for taurs like jdeer
	...
	if(backl)
		if(backl.alternate_worn_layer == CLOAK_BEHIND_LAYER)
			...
			if(backl.experimental_onback && !hide_experimental)
				...
				back_overlay = backl.build_worn_icon(age, BACK_LAYER, 'modular_rmh/icons/clothing/onmob/back_l.dmi')
				if(LAZYACCESS(offsets, OFFSET_BACK))
					back_overlay.pixel_x += offsets[OFFSET_BACK][1]
					back_overlay.pixel_y += offsets[OFFSET_BACK][2]
				// Apply taur body offset to adjust rendering height for taurs like jdeer
...
/mob/living/carbon/proc/get_sleeves_layer(obj/item/I,sleeveindex,layer2use)
	...
	if(gender == FEMALE ^ dna.species.swap_female_clothes)
		index += "_f"
