...
/obj/machinery/essence/proc/pull_from_linked(datum/essence_storage/into_storage)
	for(var/datum/essence_link/link in links)
		...
		for(var/etype in src_store.snapshot())
			...
			if(moved > 0)
				source.create_essence_flow_effect(src, etype, moved)
				break
...
/obj/machinery/essence/proc/create_essence_flow_effect(obj/machinery/target, essence_type, amount)
	var/turf/source_turf = resolve_beam_turf(src)
	var/turf/target_turf = resolve_beam_turf(target)
...
/obj/machinery/essence/proc/show_link_menu(mob/user)
	...
	for(var/datum/essence_link/link in links)
		...
		if(link.source == src)
			outbound++
	if(outbound)
		opts["View outbound links ([outbound])"] = "out"
	if(inbound)
		opts["View inbound links ([inbound])"] = "in"
	if(links.len)
		opts["Remove all links"] = "clear"
	if(!opts.len)
	...
	if(!choice || QDELETED(src) || QDELETED(user))
		return
	if(!Adjacent(user) && !istype(src, /obj/machinery/essence/cauldron_node))
		return
	switch(opts[choice])
		if("out")
			show_link_list(user, "outbound", TRUE)
		if("in")
			show_link_list(user, "inbound", FALSE)
		if("clear")
...
/obj/machinery/essence/proc/show_link_list(mob/user, label, outbound)
	...
	if(!Adjacent(user) && !istype(src, /obj/machinery/essence/cauldron_node))
		return
	var/datum/essence_link/selected_link = display[choice]
	if(QDELETED(selected_link) || !(selected_link in links))
		return
	essence_remove_link(selected_link)
	to_chat(user, span_info("Link removed."))

/obj/machinery/essence/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(user.cmode || !istype(tool, /obj/item/essence_vial))
		return NONE
	return handle_vial(tool, user)
...
/obj/machinery/essence/proc/check_vial_menu_valid(mob/user, obj/item/essence_vial/vial)
	return !QDELETED(user) && !QDELETED(vial) && Adjacent(user) && (vial in user.contents) \
