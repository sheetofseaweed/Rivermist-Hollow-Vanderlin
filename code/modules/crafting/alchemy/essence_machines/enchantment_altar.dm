...
/obj/machinery/essence/enchantment_altar/Destroy()
	...
	if(enchantment_timer)
		deltimer(enchantment_timer)
		enchantment_timer = null
	if(placed_item)
		placed_item.forceMove(drop_location())
	placed_item = null
	QDEL_NULL(selected_recipe)
	return ..()

/obj/machinery/essence/enchantment_altar/handle_atom_del(atom/deleted_atom)
	. = ..()
	if(deleted_atom != placed_item)
		return
	placed_item = null
	enchanting = FALSE
	if(enchantment_timer)
...
/obj/machinery/essence/enchantment_altar/proc/show_main_menu(mob/user)
	...
	if(!choice || choice == "cancel" || !user.default_can_use_topic(src))
		return
	switch(opts[choice])
...
/obj/machinery/essence/enchantment_altar/proc/begin_enchantment(mob/living/user)
	...
	enchantment_timer = addtimer(CALLBACK(src, PROC_REF(complete_enchantment), selected_recipe.type, user_ref), enchantment_time, TIMER_STOPPABLE)

/obj/machinery/essence/enchantment_altar/proc/complete_enchantment(epath, datum/weakref/user_ref)
	enchantment_timer = null
	if(!placed_item)
		enchanting = FALSE
		update_appearance(UPDATE_OVERLAYS)
...
/obj/machinery/essence/enchantment_altar/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	var/parent_result = ..()
	if(parent_result)
