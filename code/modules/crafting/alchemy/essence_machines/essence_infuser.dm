...
/obj/machinery/essence/infuser/handle_atom_del(atom/deleted_atom)
	. = ..()
	if(deleted_atom != infusion_target)
		return
	infusion_target = null
	infusing = FALSE
	cancel_infusion_timers()
...
/obj/machinery/essence/infuser/proc/begin_infusion(mob/living/user)
	...
	spark_timer = addtimer(CALLBACK(src, PROC_REF(infusion_spark)), 1 SECONDS, TIMER_LOOP | TIMER_STOPPABLE)
	completion_timer = addtimer(CALLBACK(src, PROC_REF(complete_infusion)), adjusted_time, TIMER_STOPPABLE)
	return TRUE

/obj/machinery/essence/infuser/proc/cancel_infusion_timers()
	if(completion_timer)
		deltimer(completion_timer)
...
/obj/machinery/essence/infuser/proc/complete_infusion()
	...
	if(!infusing) // Guard against double-fire
		return
	if(!current_recipe || !infusion_target || QDELETED(infusion_target))
		infusing = FALSE
		update_appearance(UPDATE_OVERLAYS)
		return
	infusing = FALSE
...
/obj/machinery/essence/infuser/proc/show_recipe_selection(mob/user)
	...
	for(var/rpath in subtypesof(/datum/infusion_recipe))
		...
		if(!infusion_target || !istype(infusion_target, r.target_type))
			qdel(r)
			continue
		opts[r.name] = rpath
		mapping[rpath] = r
	...
	if(!choice || !user.default_can_use_topic(src) || !infusion_target)
		for(var/rpath in mapping)
			qdel(mapping[rpath])
		return
	if(current_recipe)
...
/obj/machinery/essence/infuser/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	var/parent_result = ..()
	if(parent_result)
	...
	if(!infusion_target && !infusing)
		...
		for(var/rpath in subtypesof(/datum/infusion_recipe))
			var/datum/infusion_recipe/r = new rpath
			if(istype(tool, r.target_type))
				valid = TRUE
				qdel(r)
				break
			qdel(r)
		if(!valid)
			to_chat(user, span_warning("[tool] cannot be infused."))
