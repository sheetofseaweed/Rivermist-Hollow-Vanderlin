...
/datum/essence_combo/spell/remove(obj/item/clothing/gloves/essence_gauntlet/gauntlet, mob/living/user)
	for(var/datum/action/cooldown/spell/spell in user.actions)
		if(spell.target == gauntlet && (spell.type in granted_spells))
			qdel(spell)
