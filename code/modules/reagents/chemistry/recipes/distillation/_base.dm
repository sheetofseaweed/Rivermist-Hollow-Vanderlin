...
/proc/find_distillation_recipe(separating_reagent_type, datum/reagents/mixture)
	if(!length(GLOB.distillation_recipes) || !GLOB.distillation_recipes[separating_reagent_type])
		return null
	for(var/datum/distillation_recipe/R in GLOB.distillation_recipes[separating_reagent_type])
