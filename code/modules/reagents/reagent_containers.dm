...
/obj/item/reagent_containers/process()
	...
	// Apply only the fraction consumed by this tick. Reacting the full holder before
	// removing one percent would multiply preservation and poison effects enormously.
	var/soak_amount = max(0.2, reagents.total_volume * 0.01)
	var/datum/reagents/soak_holder = new /datum/reagents(soak_amount)
	soak_holder.my_atom = soaking_item
	reagents.trans_to(soak_holder, soak_amount, 10, 1, 1)
	soak_holder.chem_temp = reagents.chem_temp
	soak_holder.handle_reactions()
	soak_holder.reaction(soaking_item, TOUCH, 1)
	qdel(soak_holder)
...
/obj/item/reagent_containers/proc/try_feed(mob/living/user, mob/living/target)
	...
	if(ishuman(user) && istype(src, /obj/item/reagent_containers/glass/bowl))
		...
		// A bowl becomes dirtier per finished serving, not per sip.
		if(bowl_check.reagents.total_volume <= amount_per_transfer_from_this && bowl_check.reagents.get_reagent_amount(/datum/reagent/water) != bowl_check.reagents.total_volume)
			bowl_check.usages += 1
	...
	addtimer(CALLBACK(src, PROC_REF(transfer_feed_reagents), target, user), 5)
	playsound(target, pick(drinksounds), 100, TRUE)

	return TRUE

/obj/item/reagent_containers/proc/transfer_feed_reagents(mob/living/target, mob/living/feeder)
