...
/datum/round_event_control/xylix_mocking/canSpawnEvent(players_amt, gamemode, fake_check)
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(!istype(H) || H.stat == DEAD || !H.client || is_burgmeister_job(H.mind?.assigned_role))
			continue
...
/datum/round_event/xylix_mocking/start()
	...
	for(var/mob/living/carbon/human/human_mob in GLOB.player_list)
		if(!istype(human_mob) || human_mob.stat == DEAD || !human_mob.client || is_burgmeister_job(human_mob.mind?.assigned_role))
			continue
