...
/datum/erp_vfx_service/proc/play_suck(mob/living/carbon/human/user, datum/erp_sex_link/best)
	...
	if(user.gender == FEMALE)
		playsound(user,
			pick('sound/misc/mat/girlmouth (1).ogg','sound/misc/mat/girlmouth (2).ogg','sound/misc/mat/guymouth (1).ogg','sound/misc/mat/guymouth (2).ogg'),
			25, TRUE,
			ignore_walls = FALSE
		)
	else
		playsound(user,
			pick('sound/misc/mat/guymouth (2).ogg','sound/misc/mat/guymouth (3).ogg','sound/misc/mat/guymouth (4).ogg','sound/misc/mat/guymouth (5).ogg'),
			35, TRUE,
			ignore_walls = FALSE
		)
	...
	playsound(user,
		pick('sound/misc/mat/mouthend (1).ogg','sound/misc/mat/mouthend (2).ogg'),
		volume_layer, TRUE, -2,
		ignore_walls = FALSE
	)
...
/datum/erp_vfx_service/proc/try_bed_break(datum/erp_sex_link/L, mob/living/user, atom/movable/target, time)
	...
	var/obj/structure/bed/bed = find_bed_for_thrust(L, user, target)
	if(!bed || QDELETED(bed))
		return
...
/datum/erp_vfx_service/proc/find_bed_for_thrust(datum/erp_sex_link/L, mob/living/user, atom/movable/target)
	...
	var/obj/structure/bed/bed = null
