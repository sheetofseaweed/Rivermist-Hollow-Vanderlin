/datum/erp_sex_organ/penis
	...
	var/have_knot = FALSE
	var/obj/item/organ/genitals/penis/source_organ
	var/erect_mode = "auto"
...
/datum/erp_sex_organ/penis/New(obj/item/organ/genitals/penis/P)
	. = ..()
	source_organ = P
	host = P
...
/datum/erp_sex_organ/penis/proc/refresh_from_organ(obj/item/organ/genitals/penis/P)
	if(!P)
		return
	...
	var/obj/item/organ/genitals/filling_organ/testicles/T = H.getorganslot(ORGAN_SLOT_TESTICLES)
	if(!T)
		return

	var/ball_size = T.organ_size
	...
	producing.producing_reagent = T.reagent_to_make || /datum/reagent/consumable/cum
...
/obj/item/organ/genitals/penis/on_arousal_changed()
...
/obj/item/organ/genitals/penis/proc/refresh_sex_organ()
	if(!sex_organ)
...
/obj/item/organ/genitals/penis/proc/set_manual_erect_state(state)
	manual_erection_override = TRUE
	erect_state = state
	if(owner)
		owner.update_body_parts(TRUE)

/obj/item/organ/genitals/penis/proc/disable_manual_erect()
