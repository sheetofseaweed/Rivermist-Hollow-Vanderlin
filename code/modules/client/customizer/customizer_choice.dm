...
/datum/customizer_choice/proc/generate_pref_choices(list/dat, datum/preferences/prefs, datum/customizer_entry/entry, customizer_type)
	...
	if(accessory)
		...
		dat += "<div class='accessory-box'>"
		dat += "<div class='accessory-frame'>"

		dat += "<div class='accessory-preview'>"
		dat += "<img src='\ref[initial(accessory.icon)]?state=[initial(accessory.icon_state)]'/>"
		dat += "</div>"

		dat += "<div class='accessory-controls'>"
		...
		if(entry.show_dropdown && length(sprite_accessories) > 1)
			dat += "<div class='accessory-dropdown'>"
