...
/mob/living/simple_animal/hostile/retaliate/wolf_undead
//I'm not inhereting base wolf either because it uses cursed elements and AI.
	icon = 'modular_rmh/icons/mob/monster/wolf_undead.dmi'
	name = "deadite volf"
	...
	botched_butcher_results = list(/obj/item/alch/viscera = 1, /obj/item/alch/sinew = 1, /obj/item/alch/bone = 2)
...
/mob/living/simple_animal/hostile/retaliate/wolf_undead/Initialize()
	. = ..()
...
/mob/living/simple_animal/hostile/retaliate/wolf_undead/death()
	if(is_downed)
...
/mob/living/simple_animal/hostile/retaliate/wolf_undead/proc/reanimation()
...
/mob/living/simple_animal/hostile/retaliate/wolf_undead/apply_damage(damage, damagetype, def_zone, blocked, forced)
	. = ..()
	if(is_downed)
		if(def_zone == "head" || \
		   def_zone == "nose" || \
...
/mob/living/simple_animal/hostile/retaliate/wolf_undead/get_sound(input)
	switch(input)
		if("aggro")
...
/mob/living/simple_animal/hostile/retaliate/wolf_undead/simple_limb_hit(zone)
	if(!zone)
		return ""
	switch(zone)
