...
/obj/item/hair_dye_cream/attack(mob/living/M, mob/living/user)
    ...
    var/list/hairs = H.get_hair_color()
    switch(chosen)
        if("hair color")
            var/new_hair_color = color_pick_sanitized_lumi(user, "Choose your hair color", "Hair Color", hairs)
            if(new_hair_color)
        ...
        if("facial hair color")
            var/list/fhairs = H.get_facial_hair_color()
            var/new_facial_color = color_pick_sanitized_lumi(user, "Choose your facial hair color", "Facial Hair Color", fhairs)
            if(new_facial_color)
                if(!do_after(user, 30 SECONDS, target = H))
                    to_chat(user, span_warning("The dyeing was interrupted!"))
                    return
        ...
        if("natural gradient color")
            var/new_gradient_color = color_pick_sanitized_lumi(user, "Choose your natural gradient color", "Natural Gradient Color", hairs)
        ...
        if("dye gradient color")
            var/new_gradient_color = color_pick_sanitized_lumi(user, "Choose your dye gradient color", "Dye Gradient Color", hairs)
            if(new_gradient_color)
                if(!do_after(user, 30 SECONDS, target = H))
                    to_chat(user, span_warning("The dyeing was interrupted!"))
                    return
